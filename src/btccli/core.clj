...
(defn create-api
  ...
  (let [session (ssh-session "umbrel" "umbrel.local" 22 password)]    
	...
	;   == Blockchain ==   
	  ...
	  (def-api session cli gettxoutproof identity [txids [blockhash]]);["txid",...] ( "blockhash" )
	...
	;  == Rawtransactions ==
	  ...
	  (def-api session cli createpsbt identity) ;[{txid:hex,vout:n,sequence:n},...] [{address:amount,...},{data:hex},...] ( locktime replaceable )
	  (def-api session cli createrawtransaction identity); [{txid:hex,vout:n,sequence:n},...] [{address:amount,...},{data:hex},...] ( locktime replaceable )
	  ...
	  (def-api session cli signrawtransactionwithkey identity [hexstring privatekeys]) ;[( [{txid:hex,vout:n,scriptPubKey:hex,redeemScript:hex,witnessScript:hex,amount:amount},...] sighashtype )
	  (def-api session cli testmempoolaccept identity [rawtxs [maxfeerate]])
	  (def-api session cli utxoupdatepsbt identity [psbt]) ; ( [,{desc:str,range:n or [n,n]},...] )
	...
	;  == Wallet ==
	  ...
	  (def-api session cli lockunspent identity [unlock]) ;( [{txid:hex,vout:n},...] )
	  ...
	  (def-api session cli _send identity)  ;[{address:amount,...},{data:hex},...] ( conf_target estimate_mode fee_rate options )
	  (def-api session cli sendmany identity) ;  {address:amount,...} ( minconf comment [address,...] replaceable conf_target estimate_mode fee_rate verbose )
	  ...
	  (def-api session cli signrawtransactionwithwallet identity [hexstring]) ;( [{txid:hex,vout:n,scriptPubKey:hex,redeemScript:hex,witnessScript:hex,amount:amount},...] sighashtype )
	  (def-api session cli unloadwallet identity [[wallet_name load_on_startup]])
	  (def-api session cli upgradewallet identity [[version]])
	  (def-api session cli walletcreatefundedpsbt identity)  ;( [{txid:hex,vout:n,sequence:n},...] ) [{address:amount,...},{data:hex},...] ( locktime options bip32derivs )
